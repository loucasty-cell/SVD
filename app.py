...
def plot_svd_geometry(svd):
    ...
    for idx, (ax, stage, title, color) in enumerate(zip(axes, stages, titles, clrs)):
        ...
        if idx == 0:
            for vec, lbl, c in [
                (Vt2 @ v1c, r"$V^T v_1$", PALETTE["dark_blue"]),
            ...
            for vec, lbl, c in [
                ...
                (
                    S2 @ Vt2 @ v2c,
                    rf"$\sigma_2={sig[1]:.2f}$" if len(sig) > 1 else "",
                    PALETTE["red"],
                ),
            ]:
    ...
    fig.suptitle(
        "Geometric Action of SVD: Unit Circle to Ellipse",
        fontsize=13,
        fontweight="bold",
        y=1.02,
    )
...
PRESETS = {
    "Example 1 -- Slide 7.2 & 7.4 (sigma=sqrt(45), sqrt(5))": np.array(
        [[3.0, 0.0], [4.0, 5.0]]
    ),
    "Example 2a -- PS1  [[0,4],[0,0]]": np.array([[0.0, 4.0], [0.0, 0.0]]),
    "Example 2b -- PS1  [[0,4],[1,0]]": np.array([[0.0, 4.0], [1.0, 0.0]]),
    "Example 3 -- PS2  [[2,2],[-1,1]]": np.array([[2.0, 2.0], [-1.0, 1.0]]),
    "Example 4 -- Rank-1 / Pseudoinverse  [[1,1],[1,1]]": np.array(
        [[1.0, 1.0], [1.0, 1.0]]
    ),
    "Custom Matrix": None,
}
...
def matrix_input_widget(rows, cols, default, key_prefix):
    ...
    for r in range(rows):
        ...
        for c in range(cols):
            dv = (
                float(default[r, c])
                if r < default.shape[0] and c < default.shape[1]
                else 0.0
            )
...
def display_matrix_latex(M, name="M", dp=4):
    M_r = np.round(M, dp)
    rows_str = r" \\ ".join(" & ".join(f"{v:.{dp}f}" for v in row) for row in M_r)
...
def main():
    ...
    with st.sidebar:
        ...
        default_A = (
            PRESETS[preset_name] if PRESETS[preset_name] is not None else np.eye(2)
        )
        st.markdown("---")
        c1, c2 = st.columns(2)
    ...
    with tab1:
        ...
        with st.expander("Step 1: Compute A^T A and A A^T", expanded=True):
            st.latex(
                r"A^T A \in \mathbb{R}^{n \times n} \qquad A A^T \in \mathbb{R}^{m \times m}"
            )
        ...
        with st.expander("Step 2: Eigenvalues to Singular Values", expanded=True):
            st.latex(
                r"A^T A v_i = \lambda_i v_i \qquad \sigma_i = \sqrt{\lambda_i} \geq 0"
            )
        ...
        with st.expander(
            "Step 3: Right Singular Vectors V (eigenvectors of A^T A)", expanded=True
        ):
            st.latex(r"A^T A v_i = \lambda_i v_i \Rightarrow V = [v_1 | \cdots | v_n]")
            c1, c2 = st.columns(2)
            with c1:
                display_matrix_latex(svd["V"], "V")
            with c2:
                display_matrix_table(svd["V"], f"V ({svd['n']}x{svd['n']})")
            oe = np.linalg.norm(svd["Vt"] @ svd["V"] - np.eye(svd["n"]), "fro")
            ok = oe < 1e-8
            st.markdown(
                f"Orthonormality ||V^TV - I||_F = {oe:.2e}  " + badge(ok, "VTV=I"),
                unsafe_allow_html=True,
            )

        with st.expander(
            "Step 4: Left Singular Vectors U (via u_i = A v_i / sigma_i)", expanded=True
        ):
            st.latex(r"u_i = \frac{A v_i}{\sigma_i}, \quad i = 1, \ldots, r")
            c1, c2 = st.columns(2)
            with c1:
                display_matrix_latex(svd["U"], "U")
            with c2:
                display_matrix_table(svd["U"], f"U ({svd['m']}x{svd['m']})")
            if svd["rank"] > 0:
                rows_ver = []
                ...
                st.dataframe(
                    pd.DataFrame(rows_ver).set_index("i"), use_container_width=True
                )
            ue = np.linalg.norm(svd["U"] @ svd["U"].T - np.eye(svd["m"]), "fro")
            st.markdown(
                f"Orthonormality ||UU^T - I||_F = {ue:.2e}  "
                + badge(ue < 1e-8, "UUT=I"),
                unsafe_allow_html=True,
            )

        with st.expander(
            "Step 5: Full Sigma and Verification A = U Sigma V^T", expanded=True
        ):
            ...
            st.markdown(
                f"Frobenius error ||A - USigmaVT||_F = {err:.2e}  "
                + badge(err < 1e-8, "A=USigmaVT"),
                unsafe_allow_html=True,
            )
        ...
        if svd["rank"] == 0:
            ...
            st.dataframe(
                pd.DataFrame(rows_cum).set_index("k"), use_container_width=True
            )

        st.divider()
        st.markdown("### 1.3 -- Four Fundamental Subspaces")
        st.latex(
            r"\mathbb{R}^n = C(A^T) \oplus N(A) \qquad \mathbb{R}^m = C(A) \oplus N(A^T)"
        )
        sub = svd["subspaces"]
        sub_info = [
    ...
    with tab2:
        ...
        st.latex(
            r"x \xrightarrow{V^T} V^T x \xrightarrow{\Sigma} \Sigma V^T x \xrightarrow{U} Ax"
        )
        if svd["m"] < 2 or svd["n"] < 2:
            ...
            if len(sigma) > 1:
                mc2.metric("sigma_2", f"{sigma[1]:.4f}")
            ...
            st.caption(
                "Unit circle maps to an ellipse with semi-axes sigma_1*u_1, sigma_2*u_2."
            )

        st.divider()
        st.markdown("### 2.2 -- Polar Decomposition A = Q S")
        st.latex(
            r"Q = UV^T \;(\text{orthogonal}) \qquad S = V\Sigma V^T \;(\text{symmetric PSD})"
        )
        if svd["m"] != svd["n"]:
            ...
            try:
                ...
                pd_res = {
                    ...
                    "S": S_p,
                    "det_Q": (
                        float(np.linalg.det(Q_p))
                        if Q_p.shape[0] == Q_p.shape[1]
                        else 0.0
                    ),
                ...
                st.error(str(e))
                pd_res = compute_polar_decomposition(
        ...
        with c2:
            ...
            st.caption(
                f"Eigenvalues of S: {np.round(np.sort(eigs_S)[::-1], 4)}  {'PSD OK' if psd_ok else 'FAIL'}"
            )
        qs_err = pd_res["QS_error"]
        st.markdown(
            f"Verification ||QS - A||_F = {qs_err:.2e}  "
            + badge(qs_err < 1e-8, "A=QS"),
            unsafe_allow_html=True,
        )
        ...
        st.latex(
            r"A^+ = V\Sigma^+ U^T \qquad \Sigma^+_{ii} = 1/\sigma_i \;(\text{if } \sigma_i \neq 0)"
        )
        pi_res = compute_pseudoinverse(
            svd["U"], svd["singular_values"], svd["Vt"], svd["m"], svd["n"]
        )
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("**Sigma+ (inverted diagonal)**")
            display_matrix_latex(pi_res["Sigma_plus"], r"\Sigma^+")
            display_matrix_table(
                pi_res["Sigma_plus"], f"Sigma+ ({svd['n']}x{svd['m']})"
            )
        with c2:
        ...
        with c1:
            ...
            st.markdown(
                f"||AA+A - A||_F = {pi_res['err_prop1']:.2e}  " + badge(v1_ok),
                unsafe_allow_html=True,
            )
        with c2:
            st.latex(r"(2)\quad A^+ A A^+ = A^+")
            st.markdown(
                f"||A+AA+ - A+||_F = {pi_res['err_prop2']:.2e}  " + badge(v2_ok),
                unsafe_allow_html=True,
            )
    ...
    with tab3:
        ...
        st.latex(
            r"A = U\Sigma V^T \;\Rightarrow\; A_k = \sum_{i=1}^k \sigma_i u_i v_i^T"
        )
        st.markdown("### 3.0 -- Image Source")
        img_source = st.radio(
            "Image source:", ["Built-in sample", "Upload image"], horizontal=True
        )
        img_gray = None
        if img_source == "Built-in sample":
            ...
            uploaded = st.file_uploader(
                "Upload (PNG/JPG/BMP)", type=["png", "jpg", "jpeg", "bmp"]
            )
            if uploaded:
        ...
        st.dataframe(
            pd.DataFrame(rows_thr).set_index("Energy Threshold"),
